...
- `return_to_position` 的 target 可传玩家明确给出的完整 `{x,y,z}`，也可在玩家只指定高度时传 `{y}`；只传 y 时服务端优先使用已记录矿道入口的 x/z，没有可信矿道才使用女仆当前 x/z，LLM 不得因为缺少 x/z 拒绝执行。完整坐标也只能来自玩家或 `mc_game_context` 等可信工具，禁止猜测远方地面、基地或玩家位置。它默认优先该女仆同维度最近的已记录矿道，必要时使用安全地形重规划；始终为跟随的玩家保留至少一格宽、两格高、有稳定支撑且无需传送的连续通路，不得在身后回填封路。默认 `placement_policy="safe_support_and_water_seal"`，会真实消耗普通稳定方块搭桥、补支撑或从侧面封水；不会使用沙子、沙砾、矿石块、容器等不安全材料，不封岩浆，不绕过领地保护。玩家明确禁止放置或破坏时改用 `placement_policy="disabled"`；`max_placements=0` 表示不设人工放置上限。返程默认 `timeout_ms=0`，会持续到抵达、急停或结构化安全故障
...
- mc_start_maid_action(kind=动作, args=参数)：主动寻路、矿井返程或采集；navigate 参数包含 target 且不改造地形；return_to_position 参数包含可信 target，可沿矿道安全清障、搭桥、补支撑或封水并保留玩家可走的两格高路线；harvest_blocks 对“挖某类资源”使用 block/tag selector，仅对玩家明确指定的方块坐标使用 target_pos；矿石 selector 默认整矿脉采集，附近无矿时服务端自动持续探矿；明确方向、方案或关闭探矿时再传 mining_plan
