...
- Java 侧只在当前没有正在收尾的锁定矿脉时检测女仆背包容量（可能尚未开始采矿，也可能刚挖完一条矿脉）：发现候选矿石时会按真实工具掉落模拟背包插入；尚未发现目标时则要求至少一个通用空槽。无法完整容纳下一目标掉落时以 `blocked_reason=BACKPACK_FULL`、`end_reason=SAFETY_PREEMPTED` 阻塞。已经开始收尾当前矿脉时即使背包随后变满也会继续挖完，避免留下半残矿脉。收到 `BACKPACK_FULL` 时必须给出具体方案：让女仆返回基地/玩家身边并由玩家或已有卸货流程把物品存入箱子、明确丢弃或移走物品来腾出容量后再重启 mine_ore、或终止挖矿；禁止只换 selector、原样重启或继续在同一位置挖矿
