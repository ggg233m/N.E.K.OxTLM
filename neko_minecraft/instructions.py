...
- mc_start_maid_action(kind=动作, args=参数)：主动寻路、矿井返程或采集；navigate 参数包含 target 且不改造地形；return_to_position 通常只传 destination=surface|mine_entry|player，服务端自动安全修路；harvest_blocks 对“挖某类资源”使用 block/tag selector，仅对玩家明确指定的方块坐标使用 target_pos；矿石 selector 默认整矿脉采集，附近无矿时服务端自动持续探矿；明确方向、方案或关闭探矿时再传 mining_plan
