...
public final class CancelMaidActionHandler implements MessageHandlerInterface {
    ...
    public JsonObject handle(JsonObject request, WebSocket conn) {
        ...
        response.addProperty("type", "maid_action_cancel_result");
        ...
        try {
            ...
            if (cancel.rejectionReason() != null) result.addProperty("rejection_reason", cancel.rejectionReason());
            if (cancel.status() != null) result.add("action", cancel.status());
